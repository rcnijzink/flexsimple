...
CONTAINS
	...
	subroutine resample(par_ini, par_max,par_min,incon, prec_data,temp_data,etp_data,&
...
!	real*8, dimension(:,:,:), allocatable		:: pareto_output! matrix with pareto output
	...
	integer						:: uParResample	! file unit Q validation
        real*8                                          :: fIt
...
open(uParResample, file=trim(adjustl(input_dir)) // trim(adjustl("param.txt")) &
...
forall(ii=1:size(paramset,1),optim(ii) .eqv. .TRUE.) paramset(ii)=r(ii)*(par_max(ii)-par_min(ii))+par_min(ii) 
	...
	obj_mat(:, nn)     = (/NSE, LNSE, VE, KGE, LKGE/)
...
write(uObj,formHeader) 'NSE','LNSE','VE','KGE', 'LKGE'

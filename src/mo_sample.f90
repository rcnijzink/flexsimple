...
CONTAINS
	...
	logical,dimension(17), intent(in)		            :: optim	    ! parameters to optimize
	...
	real*8,dimension(17)             		            :: parbest      ! best set of parameters
   ...
   do nn=1, Iterations
    ...
    forall(ii=1:size(paramset,1),optim(ii) .eqv. .TRUE.) paramset(ii)=r(ii)*(par_max(ii)-par_min(ii))+par_min(ii) 
        ...
        paramset(4)=r2*(par_max(4)-paramset(17))+paramset(17)

           !reading in root zone data
           if(read_data .eqv. .TRUE.) then 
              !make a timeserie of Imax
              call gen_sumax(paramset(15:17), paramset(3), ichange_start, ichange_end, prec_data( iw_start:iv_end ), imax_data2 )
	      ...
	      ,temp_data(iw_start:iv_end),etp_data(iw_start:iv_end), dem, cellsize, output,sumax_data, imax_data2)
           ...
           ! else make a timeserie of sumax
               ...
               !call the model
               call model(paramset(1:14),incon, prec_data( iw_start:iv_end ) &
...
do i=1, Iterations
        ...
        write(uParam,'(17(1X f10.5))')  par_mat(:,i)
